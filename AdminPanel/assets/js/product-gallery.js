...
(function (ShopAdmin) {
  ...
  const refreshProductList = () => {
    ...
    if (categoryFilter) {
      filteredProducts = filteredProducts.filter((p) => p.categoryId === Number(categoryFilter));
    }
    if (search) {
  ...
  const saveGallery = async () => {
    ...
    if (!galleryImages.length) {
      ShopAdmin.ui.showToast('error', 'گالری نمی‌تواند خالی باشد.');
      return;
    }
    if (!galleryImages.some((i) => i.isPrimary)) {
      galleryImages[0].isPrimary = true;
    }
